...
export function PartnerForm() {
  ...
  const titolareCfFields = legalFields.filter(
    (field) => field.key === "codiceFiscale",
  );
  const titolareBirthFields = legalFields.filter((field) =>
    ["dataNascita", "luogoNascita"].includes(field.key),
  );
  ...
  return (
    ...
    >
      ...
      <div className="form-section form-section--legal">
        ...
        <div className="input-row">
          {titolareBirthFields.map((field) => (
            <div className="input-group" key={field.key}>
              <label htmlFor={field.key}>{field.label}</label>
              {field.key === "dataNascita" ? (
                <input
                  id={field.key}
                  name={field.key}
                  type="text"
